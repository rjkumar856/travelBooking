...
if(isset($_POST['SubmitButton'])){
    try{
		...
		if(empty($title) || empty($description) || empty($seating) || empty($two_way) || $_FILES["result_file"]["size"]<1){
            ...
            $FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                if($FileType == 'jpg' || $FileType == 'jpeg' || $FileType == 'png' || $FileType == 'gif'){
                    ...
                    if (move_uploaded_file($_FILES["result_file"]["tmp_name"], $target_file_new)){
                        ...
                        $stmt = $reg_user->runQuery("INSERT INTO airport_taxi(title,description,seating,two_way,city_airport,image) 
            		    VALUES('$title','$description','$seating','$two_way','$city_airport','$file_name')");
                        $stmt->execute();
                		...
                		header("Location: add-popular-vehicle-airport");
                		exit();
                		exit();
                    } else {
						...
						<div class="box box-block bg-white">
							...
							<form name="news upload" id="news" action="" method="POST" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="city_airport" id="city_airport" placeholder="Enter the City airport" value="<?php if(isset($_POST['city_airport'])){ echo $_POST['city_airport']; } ?>" />
								</div>
								</div>
								...
								<div class="form-group">
									...
									<input type="text " class="form-control" name="two_way" id="two_way" placeholder="Enter the Two Way" value="<?php if(isset($_POST['two_way'])){ echo $_POST['two_way']; } ?>" required="required"/>
								</div>

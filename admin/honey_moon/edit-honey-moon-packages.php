...
$edit_requested_id=$_GET['id'];
    ...
    header("Location: view-honey-moon-packages");
}
		...
		if(empty($title) || empty($description) || empty($no_of_days) || empty($deluxe) || empty($luxury) || empty($star) ){
            ...
            $FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                if($FileType == 'jpg' || $FileType == 'jpeg' || $FileType == 'png' || $FileType == 'gif'){
                    ...
                    $stmt = $reg_user->runQuery("UPDATE honey_moon SET title='$title',description='$description',no_of_days='$no_of_days',deluxe='$deluxe',luxury='$luxury',star='$star',image='$file_name' WHERE id='$edit_requested_id'");
                    $stmt->execute();
            		$_SESSION['add_user_error_msg'] = "<div class='alert alert-success'><button class='close' data-dismiss='alert'>&times;</button><strong>HoneyMoon Tour Pakages!</strong> Updated Successfully.</div>";
            		header("Location: edit-honey-moon-packages?id=".$edit_requested_id);
            		exit();
                    } else {
		    ...
		    $stmt = $reg_user->runQuery("UPDATE honey_moon SET title='$title',description='$description',no_of_days='$no_of_days',deluxe='$deluxe',luxury='$luxury',star='$star' WHERE id='$edit_requested_id'");
            $stmt->execute();
...
$stmt_user = $reg_user->runQuery("SELECT * FROM honey_moon WHERE id='$edit_requested_id'");
$stmt_user->execute();
...
<div class="site-content">
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4>Update HoneyMoon Tour Packages Details</h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="view-popular-vehicle-airport">HoneyMoon Tour Packages List</a></li>
							<li class="breadcrumb-item active"><a href="#">Edit HoneyMoon Tour Packages</a></li>
						</ol>

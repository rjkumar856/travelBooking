...
if(isset($_GET['delete_id'])){
	...
	$stmt = $reg_user->runQuery("DELETE FROM request_honeymoon_package WHERE id=".$_GET['delete_id']);
	$stmt->execute(array(":id"=>$id));
	header("Location: view-enquiry-honeymoon");
}
					...
					<div class="container-fluid">
						...
						<div class="box box-block bg-white">
							...
							<table class="table jsgrid table-striped table-bordered dataTable table-responsive" id="table-2">
								...
								<tbody>
                                    <?php
                                    $stmt = $reg_user->runQuery("SELECT * FROM request_honeymoon_package ORDER BY id");
                                    $stmt->execute();
...
{
	if(confirm('Are you sure you want to Delete this Request of HoneyMoon Package?'))
	{
		window.location.href='view-enquiry-honeymoon?delete_id='+id;
	}
}
</script>
...
if(isset($_POST['SubmitButton'])){
    try{
		...
		if(empty($title) || empty($description) || empty($every40hours) || empty($every80hours) || $_FILES["result_file"]["size"]<1){
            ...
            $FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                if($FileType == 'jpg' || $FileType == 'jpeg' || $FileType == 'png' || $FileType == 'gif'){
                    ...
                    if (move_uploaded_file($_FILES["result_file"]["tmp_name"], $target_file_new)){
                        ...
                        $stmt = $reg_user->runQuery("INSERT INTO local_taxi(title,description,every40hours,every80hours,extra_hours,extra_kms,image) 
            		    VALUES('$title','$description','$every40hours','$every80hours','$extra_hours','$extra_kms','$file_name')");
                        $stmt->execute();
                		...
                		header("Location: add-popular-vehicle-local");
                		exit();
                		exit();
                    } else {
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						...
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="view-popular-vehicle-local">Popular Vehicle List</a></li>
						...
						<div class="box box-block bg-white">
							...
							<form name="news upload" id="news" action="" method="POST" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="extra_hours" id="extra_hours" placeholder="Enter the Extra Hour" value="<?php if(isset($_POST['extra_hours'])){ echo $_POST['extra_hours']; } ?>" />
								</div>
								<div class="form-group">
									<label for="exampleInputPassword1"><b>Extra Kms</b></label>
									<input type="text" class="form-control" name="extra_kms" id="extra_kms" placeholder="Enter the Extra Kms" value="<?php if(isset($_POST['extra_kms'])){ echo $_POST['extra_kms']; } ?>" />
								</div>
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="every40hours" id="every40hours" placeholder="Enter the Seating Details" value="<?php if(isset($_POST['every40hours'])){ echo $_POST['every40hours']; } ?>" required="required"/>
								</div>
								<div class="form-group">
									<label for="exampleInputPassword1"><b>8Hrs x 80Kms*</b></label>
									<input type="text " class="form-control" name="every80hours" id="every80hours" placeholder="Enter the every80hours" value="<?php if(isset($_POST['every80hours'])){ echo $_POST['every80hours']; } ?>" required="required"/>
								</div>

...
$edit_requested_id=$_GET['id'];
    ...
    header("Location: view-popular-vehicle-one");
}
		...
		if(empty($title) || empty($description) || empty($seating) || empty($rate)){
            ...
            $FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                if($FileType == 'jpg' || $FileType == 'jpeg' || $FileType == 'png' || $FileType == 'gif'){
                    ...
                    $stmt = $reg_user->runQuery("UPDATE outstation_one_way_trip SET title='$title',description='$description',seating='$seating',rate='$rate',driver_allowance='$driver_allowance',min_km_per_day='$min_km_per_day',image='$file_name' WHERE id='$edit_requested_id'");
                    $stmt->execute();
            		...
            		header("Location: edit-popular-vehicle-one?id=".$edit_requested_id);
            		exit();
                    } else {
		    ...
		    $stmt = $reg_user->runQuery("UPDATE outstation_one_way_trip SET title='$title',description='$description',seating='$seating',rate='$rate',driver_allowance='$driver_allowance',min_km_per_day='$min_km_per_day' WHERE id='$edit_requested_id'");
            $stmt->execute();
    		$_SESSION['add_user_error_msg'] = "<div class='alert alert-success'><button class='close' data-dismiss='alert'>&times;</button><strong>Popilar Vehicle!</strong> Updated Successfully.</div>";
    		header("Location: edit-popular-vehicle-one?id=".$edit_requested_id);
    		exit();
...
$stmt_user = $reg_user->runQuery("SELECT * FROM outstation_one_way_trip WHERE id='$edit_requested_id'");
$stmt_user->execute();

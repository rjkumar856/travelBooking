...
if(isset($_GET['delete_id'])){
	...
	$stmt = $reg_user->runQuery("DELETE FROM request_outstation_oneway WHERE id=".$_GET['delete_id']);
	$stmt->execute(array(":id"=>$id));
	header("Location: view-enquiry-one");
}
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						...
						<ol class="breadcrumb no-bg mb-1">
							...
							<li class="breadcrumb-item active">Admin Outstation One Way Trip List</li>
						</ol>
						<div class="box box-block bg-white">
							...
							<table class="table jsgrid table-striped table-bordered dataTable table-responsive" id="table-2">
								...
								<tbody>
                                    <?php
                                    $stmt = $reg_user->runQuery("SELECT * FROM request_outstation_oneway ORDER BY id");
                                    $stmt->execute();
...
{
	if(confirm('Are you sure you want to Delete this Request Outstation One Way Trip?'))
	{
		window.location.href='view-enquiry-one?delete_id='+id;
	}
}
</script>
...
if(isset($_POST['SubmitButton'])){
    try{
		...
		if(empty($title) || empty($description) || empty($no_of_days) || empty($fare) || $_FILES["result_file"]["size"]<1){
            ...
            $FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                if($FileType == 'jpg' || $FileType == 'jpeg' || $FileType == 'png' || $FileType == 'gif'){
                    ...
                    if (move_uploaded_file($_FILES["result_file"]["tmp_name"], $target_file_new)){
                        ...
                        $stmt = $reg_user->runQuery("INSERT INTO pilgrimage_pack(title,description,no_of_days,fare,image) 
            		    VALUES('$title','$description','$no_of_days','$fare','$file_name')");
                        $stmt->execute();
                        
                		$_SESSION['add_user_error_msg'] = "<div class='alert alert-success'><button class='close' data-dismiss='alert'>&times;</button><strong>New Pilgrimage Tour Packages Details!</strong> Added Successfully.</div>";
                		header("Location: add-pilgrimage-packages");
                		exit();
                		exit();
                    } else {
...
<div class="site-content">
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4>Create New Pilgrimage Tour Packages</h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="view-pilgrimage-packages">Pilgrimage Tour Packages List</a></li>
							<li class="breadcrumb-item active"><a href="#">Add Pilgrimage Tour Packages</a></li>
						</ol>
						<div class="box box-block bg-white">
							...
							<form name="news upload" id="news" action="" method="POST" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="fare" id="fare" placeholder="Enter the Fare" value="<?php if(isset($_POST['fare'])){ echo $_POST['fare']; } ?>" required="required"/>
								</div>

...
$edit_requested_id=$_GET['id'];
    ...
    header("Location: view-south-india-packages");
}
...
if(isset($_POST['cus_update_user'])){
    try{
		...
		if(empty($title) || empty($description) || empty($no_of_days) || empty($deluxe) || empty($luxury) || empty($star)){
		    ...
		    $target_dir = "../assets/files/south_india/";
            ...
            $FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                if($FileType == 'jpg' || $FileType == 'jpeg' || $FileType == 'png' || $FileType == 'gif'){
                    ...
                    $stmt = $reg_user->runQuery("UPDATE south_india SET title='$title',description='$description',no_of_days='$no_of_days',deluxe='$deluxe',luxury='$luxury',star='$star',image='$file_name' WHERE id='$edit_requested_id'");
                    $stmt->execute();
            		$_SESSION['add_user_error_msg'] = "<div class='alert alert-success'><button class='close' data-dismiss='alert'>&times;</button><strong>South India Tour Packages!</strong> Updated Successfully.</div>";
            		header("Location: edit-south-india-packages?id=".$edit_requested_id);
            		exit();
                    } else {
		    ...
		    $stmt = $reg_user->runQuery("UPDATE south_india SET title='$title',	description='$description',no_of_days='$no_of_days',deluxe='$deluxe',luxury='$luxury',star='$star' WHERE id='$edit_requested_id'");
            $stmt->execute();
...
$stmt_user = $reg_user->runQuery("SELECT * FROM south_india WHERE id='$edit_requested_id'");
$stmt_user->execute();
...
<div class="site-content">
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4>Update South India Tour Packages Details</h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="view-south-india-packages">South India Tour Packages List</a></li>
							<li class="breadcrumb-item active"><a href="#">Edit South Tour Packages</a></li>
						</ol>
						...
						<div class="error-div"><?php if(isset($_SESSION['add_user_error_msg'])){ echo $_SESSION['add_user_error_msg']; unset($_SESSION['add_user_error_msg']); } ?></div>
							...
							<form class="form-horizontal" method="post" name="edit_ad" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="no_of_days" id="no_of_days" placeholder="Enter the No. of Details" value="<?php echo $row_user['no_of_days']; ?>" />
								</div>
								</div>

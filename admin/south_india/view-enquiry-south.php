...
if(isset($_GET['delete_id'])){
	...
	$stmt = $reg_user->runQuery("DELETE FROM request_south_package WHERE id=".$_GET['delete_id']);
	$stmt->execute(array(":id"=>$id));
	header("Location: view-enquiry-south");
}
					...
					<div class="container-fluid">
						...
						<div class="box box-block bg-white">
							...
							<table class="table jsgrid table-striped table-bordered dataTable table-responsive" id="table-2">
								...
								<tbody>
                                    <?php
                                    $stmt = $reg_user->runQuery("SELECT * FROM request_south_package ORDER BY id");
                                    $stmt->execute();
...
{
	if(confirm('Are you sure you want to Delete this Request of South Package?'))
	{
		window.location.href='view-enquiry-south?delete_id='+id;
	}
}
</script>
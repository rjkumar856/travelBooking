										...
										<div class="row">
										    <div class="col-xs-12 col-sm-5 col-md-4 pull-right booking-sub">
										        ...
										        <div class="trip-details">
										            <b><?php if(isset($this->session->userdata['booking'])){ echo $this->session->userdata['booking']['vehicle_name'];}else{ echo "No vehicle Selected"; } ?></b>
										            <span class="details">Outstation (Roundtrip)</span>
										            <span class="details">Pickup Date : <?php if(isset($this->session->userdata['booking'])){ echo date("d M Y", strtotime($this->session->userdata['booking']['round_start_date']));} ?></span>
                                                    <span class="details">Pickup Time : <?php if(isset($this->session->userdata['booking'])){ echo $this->session->userdata['booking']['round_pickup_time'];} ?></span>
                                                    <span class="details">No Of Days : <?php if(isset($this->session->userdata['booking'])){ echo $this->session->userdata['booking']['round_no_of_days'];} ?> Days</span>
                                                    <span class="details">No Of Cars : <?php if(isset($this->session->userdata['booking'])){ echo $this->session->userdata['booking']['enquiry_cars'];} ?></span>
                                                    <hr>
                                                    <span class="details">Basic Amount : <?php if(isset($this->session->userdata['booking'])){ echo $this->session->userdata['booking']['amount_to_pay'];} ?></span>
                                                    <span class="details">Total Amount : <?php if(isset($this->session->userdata['booking'])){ echo $this->session->userdata['booking']['total_amount'];} ?></span>
                                                    <span class="details">You Are Paying : <?php if(isset($this->session->userdata['booking'])){ echo $this->session->userdata['booking']['amount_to_pay'];} ?></span>
										        </div>
										        </div>
										    </div>
										 <div class="col-xs-12 col-sm-7 col-md-8 pull-left booking-sub">
										     ...
										     <h3 class="title">TRAVELLERS DETAILS</h3>
								                        ...
								                        <input type="hidden" name="vehicle_id" id="vehicle_id" class="form-control"  Placeholder="vehicle_id" value="<?php if(isset($this->session->userdata['booking'])){ echo $this->session->userdata['booking']['vehicle_id'];}else{ echo "1"; } ?>" required />
